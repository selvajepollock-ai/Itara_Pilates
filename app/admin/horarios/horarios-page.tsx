...
export default async function HorariosPage({
  ...
  const [{ data: classesData }, { data: enrollmentsData }, { data: holidaysData }, { data: cancelledData }] =
    await Promise.all([
      supabase
        .from('classes')
        .select(
      ...
      supabase
        ...
        .select('class_id, session_date, reason')
        .gte('session_date', toISODate(weekDates[0].date))
        .lte('session_date', toISODate(weekDates[6].date)),
    ])

  const holidayByDate = new Map((holidaysData ?? []).map((h) => [h.date, h.label]))
  const cancelledByKey = new Map(
    (cancelledData ?? []).map((c) => [`${c.class_id}_${c.session_date}`, c.reason])
  )
      ...
      <div className="mt-8 overflow-x-auto rounded-2xl border border-sand bg-white p-4">
        ...
        >
          ...
          {classes.map((c) => {
            ...
            const dateISO = dateForThisClass ? toISODate(dateForThisClass) : ''
            const cancelReason = cancelledByKey.get(`${c.id}_${dateISO}`)
            const isCancelled = cancelledByKey.has(`${c.id}_${dateISO}`)
            ...
            if (isCancelled) {
              return (
                <Link
                  ...
                  className="relative m-0.5 overflow-hidden rounded-lg border-2 border-dashed border-ink/20 bg-ink/5 px-1.5 py-1 text-[10px] leading-tight text-ink/50 transition hover:-translate-y-px"
                  style={{
                    gridColumn: col,
                    gridRow: `${startSlot + 2} / ${endSlot + 2}`,
                  }}
                >
                  <p className="truncate font-display italic text-ink/60">{c.class_types?.name}</p>
                  <p className="truncate">{formatTime(c.start_time)}</p>
                  <p className="truncate font-semibold text-clay/80">Cancelada</p>
                  {cancelReason && <p className="truncate text-ink/30">{cancelReason}</p>}
                </Link>
              )
            }
        ...
        <div className="mt-4 flex flex-wrap gap-4 border-t border-sand pt-4 text-xs">
          ...
          <span className="flex items-center gap-1.5 text-ink/60">
            <span className="h-2.5 w-2.5 rounded border-2 border-dashed border-ink/30 bg-ink/5" />
            Cancelada (fecha puntual)
          </span>
          {showFuerza && (
            <span className="flex items-center gap-1.5 text-ink/60">
